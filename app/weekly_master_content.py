...
@lru_cache(maxsize=4)
def _load_weekly_master_deals_cached(
    json_mtime_ns: int,
    csv_mtime_ns: int,
    venue_directory_mtime: int,
) -> tuple[SiteDeal, ...]:
    del json_mtime_ns, csv_mtime_ns, venue_directory_mtime
    ...
    for index, row in enumerate(json_rows, start=1):
        ...
        if venue_slug not in venues_by_slug:
            metadata = match_venue_metadata(row["venue"], venue_slug)
            venues_by_slug[venue_slug] = SiteVenue(
                ...
                phone=metadata.phone if metadata else None,
                website=metadata.website if metadata else None,
                menu_url=metadata.menu_url if metadata else None,
            )
            next_venue_id += 1
...
def load_weekly_master_deals() -> list[SiteDeal]:
    return list(
        _load_weekly_master_deals_cached(
            ...
            venue_directory_mtime_ns(),
        )
    )


def reset_weekly_master_deals_cache() -> None:
    _load_weekly_master_deals_cached.cache_clear()
